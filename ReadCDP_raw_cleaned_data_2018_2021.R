...
CDP_data_cleanded_target_2021 <- left_join(CDP_data_cleanded_target_2021_original, tmp_primary_industry, by=c('account_id')) %>%
                                 select(account_id, company_name, primary_industry, 
                                        ...
                                        `Targeted reduction from base year (%)`, `Target year`, `Target status in reporting year`, `Target coverage`) %>%
...
# 2018
# take maximum emissions per company
CDP_data_cleanded_target_2018_MRY_included_selection <- filter(CDP_data_cleanded_target_2018, Include==TRUE)
CDP_data_cleanded_target_2018_MRY_included_max <- group_by(CDP_data_cleanded_target_2018_MRY_included_selection, account_id) %>%
  summarise(MRY_inventory=max(MRY_inventory, na.rm=TRUE),
            MRY_target=max(MRY_target, na.rm=TRUE))
# calculated total emissions in dataset with quantifiable, scope 1+2 and active targets
# Calculate total
CDP_data_cleanded_target_2018_MRY_included_total1 <- select(CDP_data_cleanded_target_2018_MRY_included_selection, account_id) %>%
  summarise(Nr_targets=n()) %>%
  mutate(primary_industry="Total")
CDP_data_cleanded_target_2018_MRY_included_total2 <- summarise(CDP_data_cleanded_target_2018_MRY_included_max, 
                                                               Nr_companies = n(),
                                                               MRY_inventory=sum(MRY_inventory),
                                                               MRY_target=sum(MRY_target)) %>%
  mutate(primary_industry="Total",
         source_year=2018)
CDP_data_cleanded_target_2018_MRY_included_total <- left_join(CDP_data_cleanded_target_2018_MRY_included_total2, CDP_data_cleanded_target_2018_MRY_included_total1, by=c('primary_industry')) %>%
...
account_sector <- select(CDP_data_raw_2018_summary, `Account number`, `Primary industry`) %>% 
  rename(account_id=`Account number`,
         primary_industry=`Primary industry`) %>%
  distinct()
tmp1 = CDP_data_cleanded_target_2018_MRY_included_selection %>%
  mutate(primary_industry=ifelse(is.na(primary_industry), "Other", primary_industry))
tmp2 = left_join(CDP_data_cleanded_target_2018_MRY_included_max, account_sector, by=c('account_id')) %>%
  mutate(primary_industry=ifelse(is.na(primary_industry), "Other", primary_industry))

CDP_data_cleanded_target_2018_MRY_included_sector1 <- select(tmp1, account_id, primary_industry) %>%
  group_by(primary_industry) %>%
  summarise(Nr_targets=n())

CDP_data_cleanded_target_2018_MRY_included_sector2 <- group_by(tmp2, primary_industry) %>%
  summarise(Nr_companies =n_distinct(account_id),
            MRY_inventory=sum(MRY_inventory, na.rm=TRUE),
            MRY_target=sum(MRY_target, na.rm=TRUE)) %>%
  mutate(source_year=2018)
CDP_data_cleanded_target_2018_MRY_included_sector <- left_join(CDP_data_cleanded_target_2018_MRY_included_sector2, CDP_data_cleanded_target_2018_MRY_included_sector1, by=c('primary_industry')) %>%
  select(Nr_companies, Nr_targets, everything())
