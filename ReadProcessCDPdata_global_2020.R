...
# steps
# absolute_global_2020_data_excel
# absolute_global_2020_data
# absolute_global_2020_targets
# absolute_global_2020_targets_remove_duplicates

delete_global_companies_names = c("Sweco AB", "Techteam35")
delete_global_companies_account_ID= c(829595, 829599, 829603, 829640, 830933, 846242, 847923, 847937, 847956, 847957, 848264)

# 1 orginal data
options(scipen = 999)
# (X) Read in CDP data from Excel
absolute_global_2020_data_excel <- read_excel('data/CDP/input/CDP_CCTD_2020_abs_ER_public.xlsx', sheet = "Absolute ER")
...
absolute_global_2020_data_excel <- as.data.frame(absolute_global_2020_data_excel)
absolute_global_2020_data_excel$`Target year` <- as.integer(absolute_global_2020_data_excel$`Target year`)
absolute_global_2020_data_excel$`Target status in reporting year` <- str_trim(absolute_global_2020_data_excel$`Target status in reporting year`)

# Add Scope emissions from country dataset
Scope_data <- select(absolute_2020_data_excel, account_id, `Base year`, `Global S1`, `Global S2M`, `Global S2L`) %>% unique()
Scope_data$`Global S1` <- as.numeric(Scope_data$`Global S1`)
Scope_data$`Global S2M` <- as.numeric(Scope_data$`Global S2M`)
Scope_data$`Global S2L` <- as.numeric(Scope_data$`Global S2L`)
tst <- group_by(Scope_data, account_id, `Base year`) %>%
       summarise(`Global S1`=mean(`Global S1`, na.rm=TRUE), 
                 `Global S2M`=mean(`Global S2M`, na.rm=TRUE), 
                 `Global S2L`=mean(`Global S2L`, na.rm=TRUE))
absolute_global_2020_data_excel <- left_join(absolute_global_2020_data_excel, Scope_data, by=c("account_id", "Base year"))

# add rows necasssary from
absolute_global_2020_data_excel <- mutate(absolute_global_2020_data_excel, `Country/Region`="Global",
                                          #`Global S1`=`Base year emissions (100% of scope, excl. scope 3)`,
                                          #`Global S2L`=0,
                                          #`Global S2M`=0,
                                          `Country S1`=`Global S1`,
                                          `Country S2L`=`Global S2L`,
                                          `Country S2M`=`Global S2M`,
                                          percent_alloc=1,
                                          `Country impact (excl. scope 3)`=`Anticipated reduction in target year, excl. scope 3 (tCO2e)`)
...
absolute_global_2020_data_excel$`Country S1` <- as.numeric(absolute_global_2020_data_excel$`Country S1`)
absolute_global_2020_data_excel$`Country S2L` <- as.numeric(absolute_global_2020_data_excel$`Country S2L`)
absolute_global_2020_data_excel$`Country S2M` <- as.numeric(absolute_global_2020_data_excel$`Country S2M`)
absolute_global_2020_data_excel$`Global S1` <- as.numeric(absolute_global_2020_data_excel$`Global S1`)
absolute_global_2020_data_excel$`Global S2L` <- as.numeric(absolute_global_2020_data_excel$`Global S2L`)
absolute_global_2020_data_excel$`Global S2M` <- as.numeric(absolute_global_2020_data_excel$`Global S2M`)
absolute_global_2020_data_excel$`Base year emissions (100% of scope)` <- as.numeric(absolute_global_2020_data_excel$`Base year emissions (100% of scope)`)
absolute_global_2020_data_excel$`Base year emissions (100% of scope, excl. scope 3)` <- as.numeric(absolute_global_2020_data_excel$`Base year emissions (100% of scope, excl. scope 3)`)
absolute_global_2020_data_excel$`MRY emissions (100% of scope)` <- as.numeric(absolute_global_2020_data_excel$`MRY emissions (100% of scope)`)
absolute_global_2020_data_excel$`MRY emissions (100% of scope, excl. scope 3)` <- as.numeric(absolute_global_2020_data_excel$`MRY emissions (100% of scope, excl. scope 3)`)


# (X) TEMP: for two companies emissions are extremely high, these are deleted
absolute_global_2020_data_excel <- filter(absolute_global_2020_data_excel, !company_name%in%delete_global_companies_names)
absolute_global_2020_data_excel <- filter(absolute_global_2020_data_excel, !account_id%in%delete_global_companies_account_ID)

# TEMPORARILY DELETE
# SAP SE reduction = 100
absolute_global_2020_data_excel <- filter(absolute_global_2020_data_excel, !(account_id==16339 & `Is this a science-based target?`=="No, but we are reporting another target that is science-based"))

cat("absolute_global_2020_data_excel:\n")
nrow(absolute_global_2020_data_excel)
length(unique(absolute_global_2020_data_excel$account_id))

StatCDP(absolute_global_2020_data_excel, "original_2020")
...
absolute_global_2020_data<- mutate(absolute_global_2020_data_excel, `Country/Region`=ifelse(is.na(`Country/Region`), incorporation_country, `Country/Region`))
...
absolute_global_2020_data <- filter(absolute_global_2020_data, !`Target year`%in%c(0,NA)) %>% 
...
StatCDP(absolute_global_2020_data, "after_process_2020")

cat("absolute_global_2020_data:\n")
nrow(absolute_global_2020_data)
length(unique(absolute_global_2020_data$account_id))
...
absolute_global_2020_data <- mutate(absolute_global_2020_data, Scope_short = str_split(T_ID, "-") %>% map_chr(., 3)) %>%
                      filter(!(Scope_short=="S3"))
# set percent_alloc to 1 for company/target year/scope combination with only one Region/Country
absolute_global_2020_data1 <- group_by(absolute_global_2020_data, company_name, `Target year`, Scope_short) %>% filter(n()==1) %>% 
                       mutate(percent_alloc=1) #), 
absolute_global_2020_data2 <- group_by(absolute_global_2020_data, company_name, `Target year`, Scope_short) %>% filter(n()>1)
absolute_global_2020_data <- rbind(absolute_global_2020_data1, absolute_global_2020_data2)

# (X) calculate percent_alloc for those that are missing (X in scope name)                           
absolute_global_2020_data <- mutate(absolute_global_2020_data, Scope_short_tmp=Scope_short)
absolute_global_2020_data$Scope_short_tmp <- factor(absolute_global_2020_data$Scope_short_tmp, levels=Scopes_short)
levels(absolute_global_2020_data$Scope_short_tmp) <- list(
  S1 = c("S1", "SX", "S13"),
  S2L = c("S2L", "S2X"),                                            
  S2M = c("S2M", "S2M3"),
...
write.table(absolute_global_2020_data, "data/CDP/output/absolute_global_2020_data_tmp1.csv", sep=";", col.names = TRUE, row.names=FALSE)

absolute_global_2020_data <- mutate(absolute_global_2020_data, percent_alloc_tmp = case_when(
...
#write.table(absolute_global_2020_data, "data/CDP/output/tmp1.csv", sep=";", col.names = TRUE, row.names=FALSE)

absolute_global_2020_data <- mutate(absolute_global_2020_data, percent_alloc=ifelse(percent_alloc %in% c(0, NA), percent_alloc_tmp, percent_alloc))
write.table(absolute_global_2020_data, "data/CDP/output/absolute_global_2020_data_tmp2.csv", sep=";", col.names = TRUE, row.names=FALSE)

# (X) delete remaining percent_alloc that is zero or NA
tmp<-filter(absolute_global_2020_data, is.na(percent_alloc))
nrow(tmp)
tmp<-filter(absolute_global_2020_data, percent_alloc==0)
nrow(tmp)
tmp<-filter(absolute_global_2020_data, is.na(percent_alloc) | percent_alloc==0)
nrow(tmp)
absolute_global_2020_data <- filter(absolute_global_2020_data, !(is.na(percent_alloc) | percent_alloc==0))
nrow(absolute_global_2020_data)
# remove ; because this is inconvenient for csv files. TEMP --> does not work, so remove columsn
absolute_global_2020_data <- select(absolute_global_2020_data, -`Please explain (including target coverage)`)
write.table(absolute_global_2020_data, "data/CDP/output/absolute_global_2020_data.csv", sep=";", col.names = TRUE, row.names=FALSE)

#--------------------------------------------

# PROCESS DATA
absolute_global_2020_targets <- rename(absolute_global_2020_data, `% emissions in Scope`=`Covered emissions in base year as % of total base year emissions in selected Scope(s) (or Scope 3 category)`,
...
cols_absolute_global_2020 <- #c("company_name", "incorporation_country", "Country/Region", "sectors", "industries", "activities",
...
absolute_global_2020_targets <- select(absolute_global_2020_targets, cols_absolute_global_2020) %>% rename(Scope_long=Scope)
tc1 <- unique(absolute_global_2020_targets$`Target coverage`)
tc2 <- c("Company-wide", "Business activity", "Business division", "Site/facility","Country/region")
tc1 <- tc1[!tc1%in%tc2]
tc <- c(tc2, tc1)
absolute_global_2020_targets$`Target coverage` <- factor(absolute_global_2020_targets$`Target coverage`, levels=tc)
...
scope_info <- data.frame (
              Scope_short     = c("S12M",      "S12X3",       "S1",      "S12M3",       "S12L",      "S2M",     "S12L3",       "S3",      "S2L",     "S12X",      "SX", "S2X3",       "S13",      "S2X", "S2M3"),
              Scope           = c("Scope 1+2", "Scope 1+2+3", "Scope 1", "Scope 1+2+3", "Scope 1+2", "Scope 2", "Scope 1+2+3", "Scope 3", "Scope 2", "Scope 1+2",  "",  "Scope 2+3", "Scope 1+3", "Scope 2", "Scope 2"),
              Scope_3         = c(FALSE,       FALSE,         FALSE,     FALSE,        FALSE,       FALSE,     FALSE,         TRUE,      FALSE,     FALSE,        TRUE, FALSE,       FALSE,       FALSE, TRUE),
              Scope1          = c("S1",        "S1",           "S1",      "S1",         "S1",        "",        "S1",          "",        "",       "S1",          "",   "",          "S1",        "", ""),
              Scope2          = c("S2M",        "max",         "",        "S2M",        "S2L",       "S2M",     "S2L",         "",        "S2L",    "max",         "",   "max",       "",          "max", "S2M"))

# Determine scope for companies with X in Scope
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, Scope_short = str_split(T_ID, "-") %>% map_chr(., 3))
tmp <- filter(absolute_global_2020_targets, str_detect(Scope_short, "X"))
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, Scope_short=ifelse(!(Scope_short=="SX"), Scope_short, ifelse(Scope_short=="SX" & !(`Country S1`==0), "S1", ifelse(Scope_short=="SX" & !(`Country S2L`==0), "S2", ifelse(!(`Country S2M`==0), "S2M", Scope_short)))))
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, Scope_short=ifelse(Scope_short=="S2X", "S2", Scope_short))
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, Scope_short=ifelse(Scope_short%in%c("S12X", "S123X"), ifelse(!(`Country S2L`==0), "S2L", ifelse(!(`Country S2M`==0), "S2M", Scope_short)), Scope_short))
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, Scope_short=ifelse(Scope_short=="S2X", "S2", Scope_short))

# Determine scope (Scope 1+2, Scope 1 or Scope 2)
absolute_global_2020_targets <- left_join(absolute_global_2020_targets, scope_info, by=c('Scope_short')) %>% select(-Scope_3)
absolute_global_2020_targets$Scope <- factor(absolute_global_2020_targets$Scope, levels=Scopes)
absolute_global_2020_targets$Scope_short <- factor(absolute_global_2020_targets$Scope_short, levels=Scopes_short)

nrow(absolute_global_2020_targets)

## 3b. calculate percent_alloc for those occurences where this is zero/empty
#absolute_global_2020_targets <- mutate(absolute_global_2020_targets, `Country S2L` = ifelse(is.na(`Country S2L`), 0, `Country S2L`),
...
#absolute_global_2020_targets <- mutate(absolute_global_2020_targets, `Country S2L` = ifelse(`Country S2L`==0, `Country S2M`, `Country S2L`),
...
#check <- select(absolute_global_2020_targets, company_name, `Country/Region`, percent_alloc, percent_alloc_tmp) %>% mutate(diff_perc=round(100*(percent_alloc_tmp/percent_alloc-1),1))
#absolute_global_2020_targets <- mutate(absolute_global_2020_targets, percent_alloc=ifelse(is.na(percent_alloc) | percent_alloc==0, percent_alloc_tmp, percent_alloc)) %>%
#                         select(-N1, -N2, -D1, -D2, -percent_alloc_tmp)
#write.table(absolute_global_2020_targets, "data/CDP/output/absolute_global_2020_check_percent_alloc.csv", sep=";", col.names = TRUE, row.names=FALSE)
...
nrow(absolute_global_2020_targets)
absolute_global_2020_targets <- filter(absolute_global_2020_targets, !(Scope=="Scope 1" & `Country S1`==0 & percent_alloc%in%c(0, NA)))
absolute_global_2020_targets <- filter(absolute_global_2020_targets, !(Scope=="Scope 2" & `Country S2L`+`Country S2M`==0 & percent_alloc%in%c(0, NA)))
nrow(absolute_global_2020_targets)
...
absolute_global_2020_targets <- rename(absolute_global_2020_targets, BY_EM_target=`Base year emissions covered by target (metric tons CO2e)`,
...
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, BY_EM_target=BY_EM_target*percent_alloc,
...
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, target_type="absolute",
...
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, `BaseYearEmissions_excl_scope3`=ifelse(is.na(`BaseYearEmissions_excl_scope3`), 0, `BaseYearEmissions_excl_scope3`))
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, `MRY_EM_excl_scope3`=ifelse(is.na(`MRY_EM_excl_scope3`), 0, `MRY_EM_excl_scope3`))

# If perc_scope1 is NA change to 40% for Scope1+2, 100% for Scope 1 and 0% for Scope 2
avg_scope=1
absolute_global_2020_targets <- mutate(absolute_global_2020_targets, perc_scope1 = ifelse(!is.na(perc_scope1), perc_scope1, ifelse(Scope%in%c("Scope 1+2", "Scope 1+2+3"), avg_scope, ifelse(Scope%in%c("Scope 1","Scope 1+3"), 1, 0))))

absolute_global_2020_targets$BaseYearEmissions_excl_scope3 <- as.numeric(absolute_global_2020_targets$BaseYearEmissions_excl_scope3)
absolute_global_2020_targets$TargetYearEmissions_excl_scope3 <- as.numeric(absolute_global_2020_targets$TargetYearEmissions_excl_scope3)
write.table(absolute_global_2020_targets, "data/CDP/output/absolute_global_2020_targets.csv", sep=";", col.names = TRUE, row.names=FALSE)

cat("absolute_global_2020_targets:\n")
nrow(absolute_global_2020_targets)
length(unique(absolute_global_2020_targets$account_id))
...
tmp1 <- filter(absolute_global_2020_targets, is.na(`Most recent accounting year`))
tmp2 <- filter(absolute_global_2020_targets, is.na(`% reduction from base year`))
tmp3 <- filter(absolute_global_2020_targets, is.na(`% emissions in Scope`))
#tmp4 <- filter(absolute_global_2020_targets, is.na(Scope_long))
#tmp5 <- filter(absolute_global_2020_targets, is.na(`Country impact (excl. scope 3)`))
absolute_global_2020_remove <- rbind(tmp1, tmp2, tmp3)
write.table(absolute_global_2020_remove, "data/CDP/output/absolute_global_2020_removed.csv", sep=";", col.names = TRUE, row.names=FALSE)

nrow(absolute_global_2020_remove)
...
absolute_global_2020_targets_remove_duplicates <- mutate(absolute_global_2020_targets, `Country impact (excl. scope 3)`=ifelse(is.na(`Country impact (excl. scope 3)`), 0, `Country impact (excl. scope 3)`))
nrow(absolute_global_2020_targets_remove_duplicates)
...
cols_other <- colnames(absolute_global_2020_targets)
cols_other <- cols_other[!cols_other%in%cols_id]
# count number of duplicates
test_duplicates <- group_by_at(absolute_global_2020_targets_remove_duplicates, all_of(cols_id)) %>% 
                   summarise(c=n()) %>%
                   filter(c>1)
...
absolute_global_2020_targets_remove_duplicates$`Is this a science-based target?` <- factor(absolute_global_2020_targets_remove_duplicates$`Is this a science-based target?`, levels=Science_based)
absolute_global_2020_targets_remove_duplicates <- select(absolute_global_2020_targets_remove_duplicates, all_of(cols_id), everything()) %>%
                                           group_by_at(cols_id) %>% 
...
absolute_global_2020_targets_remove_duplicates <- select(absolute_global_2020_targets_remove_duplicates, -`Country/Region`)

cat("absolute_global_2020_targets_remove_duplicates:\n")
nrow(absolute_global_2020_targets_remove_duplicates)
length(unique(absolute_global_2020_targets_remove_duplicates$account_id))

write.table(absolute_global_2020_targets_remove_duplicates, "data/CDP/output/absolute_global_2020_targets_remove_duplicates.csv", sep=";", col.names = TRUE, row.names=FALSE)
absolute_global_2020_duplicates <- anti_join(absolute_global_2020_targets, absolute_global_2020_targets_remove_duplicates, by=cols_id)
write.table(absolute_global_2020_duplicates, "data/CDP/output/absolute_global_2020_duplicates.csv", sep=";", col.names = TRUE, row.names=FALSE)

cat("absolute_global_2020_duplicates:\n")
nrow(absolute_global_2020_duplicates)
length(unique(absolute_global_2020_duplicates$account_id))
