...
absolute_2020_targets_per_row1 <- filter(absolute_2020_targets_remove_duplicates, `Target coverage`=="Company-wide" & `% emissions in Scope`>0.75) %>%
                                  #select(cols_id_row_plus, `Country/Region`, `Target year`, `Base year`, `% reduction from base year`) %>%
                                  #mutate(`Country/Region`="tmp") %>%
                                  #group_by_at(cols_id_row_plus) %>%
                                  group_by(`account_id`, `company_name`,  `primary_sector`, `primary_industry`, `primary_activity`, `target_type`, `Scope`,`Target year`) %>%
                                  arrange(`Target year`, `Base year`) %>%  
                                  mutate(r=row_number(), 
                                         target_nr=paste0("Target_", row_number()),
                                         )
nr_targets <- group_by_at(absolute_2020_targets_per_row1, cols_id_row) %>%
                          summarise(nr_targets=sum(r==1))
