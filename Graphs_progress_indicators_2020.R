...
choice = "response" # resonse or processed
# read in data (response or processed)
# RESPONSE DATA
data_2020_response_original <- read_excel('data/CDP/input/CDP_CCTD_2020_abs_ER_public.xlsx', sheet = "Absolute ER")
data_2020_response <- select(data_2020_response_original, account_id, 
...
nrow(data_2020_response)

# PROCESSED DATA (CAAT)
data_CAAT_2020 <- read.table('data/CDP/output/data_CAAT.csv', sep=";", header = TRUE)
nrow(data_CAAT_2020)
data_2020_processed <- mutate(data_CAAT_2020, selection=ifelse(is.na(`Target.Year.2`), 1, 2),
...
data_2020_processed <- mutate(data_2020_processed, account_id=`Actor.name`,
                                                 ...
                                                 `Most recent accounting year`=`MRY.Year`) %>%
                     select(account_id, 
                            `Base year emissions (100% of scope)`, `Base year emissions (100% of scope, excl. scope 3)`, `Base year`,
                            `MRY emissions (100% of scope)`, `MRY emissions (100% of scope, excl. scope 3)`, `Most recent accounting year`,
                            `Targeted reduction from base year (%)`, `Target year`)
nrow(data_2020_processed)


if (choice=="response") {data_2020=data_2020_response} else {data_2020=data_2020_processed}

   
data_progress <-  mutate(data_2020,
                  ...
                  rename(BY=`Base year`,
                         MRY=`Most recent accounting year`,
...
data_progress <- mutate(data_progress, TY_EM=(1-TR/100)*BY_EM,
                                       ...
                                       target_year_group=ifelse(TY<=2020, 0, ifelse(TY<=2035, 1, ifelse(TY<2050, 2, 3))),
...
#d_progress=filter(data_progress, target_year_group%in%c('2'))
#progress_TY_labels=c("2036-2050")
d_progress=filter(data_progress, target_year_group%in%c('0', '1', '2', '3'))
progress_TY_labels=c("<=2020", "2021-2030", "2031-2040", ">2040")
nrow(d_progress)
...
progress_TY_labels=c( "2021-2030", "2031-2040", ">2040")
...
g_maturity_progress <- ggplot(data=filter(d_ambition_progress, maturity_remaining>0, maturity_remaining<40, annual_progress_a>-100, annual_progress_a<100),
  ...
  ylab("annualised progress between base year and most recent accounting year (%)") +
  #xlim(0, 40) +
  #ylim(-100, 100) +
  theme_bw()
...
g_ambition_progress <- ggplot(data=filter(d_ambition_progress, annual_ambition_a>-100, annual_ambition_a<100, annual_progress_a>-100, annual_progress_a<100),
  ...
  geom_point(aes(size=on_track), colour="grey20") +
  geom_point(aes(colour=target_year_group)) +
  geom_point(data=TwoC_progress, aes(x=annual_ambition_a, y=annual_progress_a, shape=temperature), colour="grey", size=6) +
  geom_point(data=OneAndAHalfC_progress, aes(x=annual_ambition_a, y=annual_progress_a, shape=temperature), colour="grey", size=6) +
  geom_vline(xintercept=TwoC_annual, colour="grey", linetype="solid", size=1.5) +
  geom_vline(xintercept=OneAndAHalfC_annual, colour="grey", linetype="solid", size=1.5) +
  geom_smooth(aes()) +
  scale_colour_discrete(name="Target year", labels=progress_TY_labels) +
  scale_size_manual(name="On track", values=c("TRUE"=5, "FALSE"=0.0)) +
